...
DEFAULT_URL = ('https://www.glassdoor.co.in/nokia')
...
def sign_in():
    ...
    submit_btn =browser.find_element_by_xpath('//button[text()="Sign In"]')
...
def main():
    ...
    while more_pages() and\
        ...
        go_to_next_page()
        reviews_df = extract_from_page()
        df=reviews_df.date.apply(lambda x: pd.Series(str(x).split(" ")))
        
        df = df.iloc[[-1]]
        df[1]=df[1].apply(lambda x : strptime(x,'%b').tm_mon)
        current_time = dt.datetime.now()  
        end_Y = current_time.year - 1
        end_M = current_time.month
       

        date_limit_reached[0] = (int(df[1])<end_M and int(df[3])== end_Y) or (int(df[3])< end_Y)
    ...
    now=str(now)[:19]
    file = "glassdoor_data_UK_"+(str(now)[:10])+"_Time_"+str(now)[11:]+".csv"
    file =file.replace(":","_")
